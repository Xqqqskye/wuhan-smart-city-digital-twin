...
这不是一个只负责回答问题的聊天窗口。用户可以直接提出一个包含多个目标的城市任务，例如：

> **“帮我查找武汉大学附近的充电桩，定位到最近的一个，并从黄鹤楼规划驾车路线。”**

城市智能体会根据中间结果继续决策并操作地图：

1. 识别“查设施、选最近地点、定位、规划路线”四个连续意图。
2. 调用高德 POI 搜索武汉大学附近的真实充电站，而不是让模型凭空编造地点。
3. 从搜索结果中选择合适的充电站并将地图定位过去，同时加载周边充电站图层。
4. 把选中的地点自动传给导航工具，规划从黄鹤楼出发的驾车路线。
5. 在 Mapbox 地图上绘制路线，并展示推荐方案、备选方案、距离、耗时、红绿灯数量和逐路段指引。

一次本地实测中，智能体找到“新电途汽车充电站（武汉大学卓尔体育馆充电站）”，随后生成了 3 条驾车方案；推荐路线约 **8.8 公里、22 分钟、15 个红绿灯**。实际结果会随 POI、路况和第三方地图服务变化。
...
同一套机制还可以扩展到“回家途中顺便充电”“下雨时优先公交地铁”“结合 AQI 和限行规划明早出行”等跨数据源、多工具城市任务。当前免费 POI 接口提供位置与基础属性，不代表实时空闲充电枪；配置运营商或政府 MCP 数据源后，智能体可以进一步加入实时可用状态。

...
const messages = ref([
  { role: 'assistant', content: '你好，我是武汉城市智能体。你可以直接告诉我想去哪里、如何导航、切换什么图层，或查看哪类城市设施。' }
]);
const suggestions = ['定位到黄鹤楼', '从黄鹤楼公交到武汉大学', '显示附近停车场', '切换城市天际线视角'];
...
async function runActions(message, actions = []) {
  ...
  if (!props.actionExecutor) {
    message.actionRuns.forEach(item => { item.status = 'error'; item.detail = '地图执行器尚未就绪'; });
    return;
  }
  for (let index = 0; index < actions.length; index += 1) {
    ...
    await nextTick();
    try {
      const result = await props.actionExecutor(actions[index]);
      run.status = 'success';
...
async function send(preset = '') {
  ...
  try {
    ...
    const response = await fetch('/api/qwen', {
      ...
      body: JSON.stringify({
        messages: messages.value.map(({ role, content: text }) => ({ role, content: text })),
        context
      })
    });
    const data = await response.json();
    ...
    await nextTick();
    messageList.value?.scrollTo({ top: messageList.value.scrollHeight, behavior: 'smooth' });
    await runActions(assistantMessage, actions);
  } catch (error) {
...
<template>
  <div class="assistant-shell" :class="{ 'city-mode': cityMode }">
    <transition name="assistant-pop">
      <section v-if="open" class="assistant-panel" aria-label="Qwen 城市智能体">
        <header>
          <div class="assistant-logo">Q</div>
          <div><strong>城市智能体</strong><small>{{ model }} · {{ configured ? '地图执行器已连接' : '等待密钥配置' }}</small></div>
        ...
        <div ref="messageList" class="messages">
          <div v-for="(message,index) in messages" :key="index" class="message" :class="[message.role,{ error:message.error }]">
            <span>{{ message.role === 'assistant' ? 'Q' : '你' }}</span>
        ...
        <div class="composer">
          <textarea v-model="input" rows="1" placeholder="例如：从黄鹤楼坐地铁到武汉大学" @keydown.enter.exact.prevent="send()"></textarea>
          <button :disabled="!input.trim() || sending" aria-label="发送消息" @click="send()">↑</button>
        </div>
        <footer>地图操作由白名单工具执行 · AI 内容请核验关键数据</footer>
      </section>
    </transition>

    <button class="assistant-trigger" :class="{ active: open }" aria-label="打开 Qwen 城市智能体" @click="open = !open">
      <span>Q</span><div><strong>QWEN</strong><small>城市智能体</small></div><i :class="{ online: configured }"></i>
    </button>
  </div>
</template>
...
.assistant-panel{width:min(410px,calc(100vw - 32px));height:min(590px,calc(100vh - 120px))}
.message-body{max-width:84%;min-width:0}.message-body>p{max-width:none}.message.user .message-body>p{border-radius:12px 4px 12px 12px;color:#fff;background:#218eaa}.message.error .message-body>p{color:#a45a35;background:#fff1e8}
.action-runs{margin-top:6px;display:grid;gap:5px}.action-run{padding:7px 9px;display:grid;grid-template-columns:8px 1fr auto;align-items:center;gap:6px;border:1px solid #d7e8ec;border-radius:8px;color:#4b6872;background:#f8fbfc;font-size:9px}.action-run i{width:7px;height:7px;border-radius:50%;background:#a8b9bf}.action-run small{color:#82969d;font-size:8px}.action-run.running i{background:#29a8c8;box-shadow:0 0 0 3px rgba(41,168,200,.12);animation:pulse 1s infinite}.action-run.success{border-color:#cce9dc;background:#f1fbf6}.action-run.success i{background:#2cc48a}.action-run.error{border-color:#f1d8ce;background:#fff7f2}.action-run.error i{background:#ef795c}.action-run.error small{color:#bf624d}
@keyframes pulse{50%{opacity:.45}}
@media(max-width:760px){.assistant-panel{width:min(410px,calc(100vw - 28px))}}
</style>
